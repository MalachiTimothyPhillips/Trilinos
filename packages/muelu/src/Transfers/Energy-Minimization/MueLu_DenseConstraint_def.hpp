...
void DenseConstraint<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Setup() {
  ...
  const LocalOrdinal numLocalRows =
      static_cast<LocalOrdinal>(numRows);

  const LocalOrdinal numPotentialConstraints =
      static_cast<LocalOrdinal>(numRows * NSDim);

  // Treat very small coefficients as zero. This is mainly to skip exactly-zero
  // canonical-nullspace constraints such as y/z translation constraints on an
  // x-displacement row.
  const magnitude_type zeroTol =
      static_cast<magnitude_type>(100) * magATS::epsilon();
  ...
  const GlobalOrdinal localPotentialConstraintsGO =
      static_cast<GlobalOrdinal>(numPotentialConstraints);

  GlobalOrdinal globalPotentialConstraints = 0;

  Teuchos::reduceAll(
      *comm,
      Teuchos::REDUCE_SUM,
      1,
      &localPotentialConstraintsGO,
      &globalPotentialConstraints);
...
DenseConstraint<Scalar, LocalOrdinal, GlobalOrdinal, Node>::FindBlocks(RCP<const Xpetra::CrsGraph<LocalOrdinal, GlobalOrdinal, Node>>& XXt) {
  // Since DenseConstraint::Setup() now omits individual zero/trivial constraint
  // rows, the old assumption of exactly NSDim consecutive constraints per fine
  // row is no longer valid. Use the generic graph-based block discovery.
  return ::MueLu::Constraint<Scalar, LocalOrdinal, GlobalOrdinal, Node>::FindBlocks(XXt);
}
