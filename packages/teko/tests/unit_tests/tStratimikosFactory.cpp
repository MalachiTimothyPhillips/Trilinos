...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_Defaults) {
  ...
  RCP<Tpetra::Operator<ST, LO, GO, NT> > eA = buildSystem(comm, 5);
...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_MultipleCalls) {
  ...
  RCP<Tpetra::Operator<ST, LO, GO, NT> > eA = buildStridedSystem(comm, 5);
  RCP<Thyra::LinearOpBase<double> > tA      = Thyra::tpetraLinearOp<ST, LO, GO, NT>(
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(eA->getRangeMap()),
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(eA->getDomainMap()), eA);
...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_BlockGaussSeidel) {
  ...
  Teko::LinearOp testOp;
  {
    ...
    RCP<Tpetra::Operator<ST, LO, GO, NT> > strided_eA =
        Teuchos::rcp(new Teko::TpetraHelpers::StridedTpetraOperator(2, eA));
    ...
    testOp = Thyra::tpetraLinearOp<ST, LO, GO, NT>(
        ...
        Thyra::tpetraVectorSpace<ST, LO, GO, NT>(invFactOp->getDomainMap()), invFactOp);
  }
