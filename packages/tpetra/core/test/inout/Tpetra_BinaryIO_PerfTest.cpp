...
makeBandedMatrix(const RCP<const Tpetra::Map<LO, GO, Node> >& map,
  ...
  auto A = rcp(new matrix_type(map, static_cast<size_t>(nnzPerRow)));
...
void runDenseCase(const Config& config,
  ...
  const RCP<const map_type> targetMap =
      config.customMap ? makeCyclicMap<LO, GO, Node>(comm, globalNumRows) : fileMap;
  ...
  for (int repeat = 0; repeat < config.numRepeats; ++repeat) {
    ...
    {
      auto timer = Teuchos::TimeMonitor::getNewTimer(config.customMap ?
                                                       "BinaryIO dense read custom map" :
                                                       "BinaryIO dense read default map");
      Teuchos::TimeMonitor timeGuard(*timer);
      Y = config.customMap ? binary_io_type::readDenseFile(filename, targetMap) :
                             binary_io_type::readDenseFile(filename, comm);
    }
...
void runSparseCase(const Config& config,
  ...
  for (int repeat = 0; repeat < config.numRepeats; ++repeat) {
    ...
    {
      auto timer = Teuchos::TimeMonitor::getNewTimer(config.customMap ?
                                                       "BinaryIO sparse read custom map" :
                                                       "BinaryIO sparse read default map");
      Teuchos::TimeMonitor timeGuard(*timer);
      B = config.customMap ? binary_io_type::readSparseFile(filename, targetMap, targetMap, targetMap, true) :
                             binary_io_type::readSparseFile(filename, comm);
    }
