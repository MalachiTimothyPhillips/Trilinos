...
void copy_dev_view_to_host_array(array_t& array, const dev_view_t& dev_view) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ext != size_t(array.size()), std::logic_error,
...
template <class MatrixType>
void MDF<MatrixType>::setMatrix(const Teuchos::RCP<const row_matrix_type>& A) {
...
MDF<MatrixType>::getPermutations() const {
  TEUCHOS_TEST_FOR_EXCEPTION(
      permutations_.is_null(), std::runtime_error,
...
MDF<MatrixType>::getReversePermutations() const {
  TEUCHOS_TEST_FOR_EXCEPTION(
      reversePermutations_.is_null(), std::runtime_error,
...
void MDF<MatrixType>::
  ...
  // global_ordinal_type.
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(fillLevel != 0, std::runtime_error, prefix << "MDF with level of fill != 0 is not yet implemented.");
  }
  {
    const std::string paramName("Verbosity");
    getParamTryingTypes<int, int, global_ordinal_type, double, float>(verbosity, params, paramName, prefix);
  }
  {
